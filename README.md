...
**Not yet built:**
- Per-type condition/action fields (replacing the raw JSON textarea) — e.g. `HasTag`'s params should be a tag autocomplete, not free text. **Implementation plan (started and reverted mid-session — do this properly, not half-done):**
  1. Add one dedicated field per known type in `ordo_campaign_form.xml` (e.g. `tag` for `HasTag`, `amount` for `OrderTotalAtLeast`), each hidden by default via `<visible>false</visible>` and shown via `<switcherConfig>` keyed off the row's `type` select (`target: ${ $.parentName }.<field>`, `callback: visible`). Keep `params_json` as the fallback for any type without a dedicated field yet.
  2. **The part that's actually missing:** `Controller/Adminhtml/Campaign/Save.php` currently reads `params_json` directly from posted `conditions[]`/`actions[]` rows. It needs to instead merge any posted dedicated fields (`tag`, `amount`, ...) into a `params` array per row *before* JSON-encoding to `params_json` — otherwise the new fields are UI-only and silently do nothing. Write this as a small `normalizeRowParams(array $row): array` method, covered by a unit test per condition/action type.
  3. Verify end-to-end against the real Magento instance (create a campaign using the dedicated field, confirm the saved `params_json` in `ordo_campaign_condition`/`ordo_campaign_action` is correct) before calling it done — this exact class of bug (UI looks right, backend never wired) is why the campaign form took so long to get right in the first place.
