...
**PHPStan — level max, zero errors, backlog tracked not hidden.** 211 pre-existing findings — overwhelmingly safe Magento idioms PHPStan's strict typing flags anyway (`Magento\Framework\Phrase` passed where `string` is type-hinted, always safe via `__toString()`; `mixed` from `AbstractModel`/`DataObject`'s magic `getData()`; missing iterable value types on `array` params) — were reviewed one by one, not blanket-suppressed: the two genuinely real issues found (`json_encode()` returning `string|false`; `EscalateStalePendingApprovals`'s defensive-but-provably-unreachable null check) were fixed or confirmed harmless, then the rest captured in `phpstan-baseline.neon` (standard PHPStan workflow for adopting strict analysis into an existing codebase) — every remaining finding is listed there by file and line, not swept under the rug. Going forward, `analyse` genuinely passes clean, so any *new* violation is caught immediately instead of drowned in 211 pre-existing ones.

**Coverage — 346 unit tests**, all passing for real against a live instance. The ~98% class / ~99.5% method figures in `VERIFICATION.md` predate the free-gift feature and haven't been re-measured via PCOV since — due for a refresh, not a claim about current numbers.
