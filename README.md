...
**Not yet built:**
- Per-type condition/action fields (replacing the raw JSON textarea) — e.g. `HasTag`'s params should be a tag autocomplete, not free text. **Attempted twice, reverted both times — do this properly, not half-done:**
  - **Attempt 1** (documented previously): backend wiring (`Save.php` merging dedicated fields into `params_json`, `DataProvider.php` decoding them back out for edit) was never written — UI-only, would've silently done nothing.
  - **Attempt 2** (this session): wrote the full pair — `<field>` per type (`tag`, `amount`, `rule_id`, `prefix`, `template`, `message`) hidden via `<visible>false</visible>` and shown via `<switcherConfig>` (`target: ${ $.parentName }.<field>`, `callback: visible`, keyed off the row's `type` select value) in `ordo_campaign_form.xml`, **plus** the backend merge in `Save.php`/`DataProvider.php`. Backend logic is straightforward and untested-but-sound. **The blocker is purely front-end:** confirmed live (real admin, real browser, cache cleared) that selecting a `type` value never toggles the matching field's visibility — verified via `getComputedStyle().display` on the field wrapper staying `none` even after manually dispatching a native `change` event on the underlying `<select>`. Not root-caused before reverting; candidates worth checking first:
    - `switcherConfig` may need `imports`/`exports` wiring instead of (or in addition to) `rules`/`actions` — the version used here follows the shape from Magento core's attribute-options-select modal, which is a `dynamicRows` too, but not nested inside a `container` with `is_collection`/`isTemplate` the way this form's `record` container is — that nesting may change what `${ $.parentName }` actually resolves to.
    - Try `ko.dataFor()`/`ko.contextFor()` on the `type` select's DOM node in the browser console to see its actual component `name`/`index`, and compare against what `${ $.parentName }.tag` is supposed to expand to, rather than guessing.
    - Rule this out first: does switching `type` even fire the record-level UI event at all (add a `console.log` via a quick `imports` on a scratch field) before debugging the visibility callback specifically.
