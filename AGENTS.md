...
**W trakcie diagnozy (nieukończone):** po naprawieniu powyższych trzech, `AdminCampaignScenarioEndToEndTest` jako jedyny wciąż failuje — formularz kampanii zapisuje się w 100% poprawnie (`enabled=1`, trigger/condition/action w bazie dokładnie zgodne z tym co wpisano w UI, potwierdzone bezpośrednim dumpem `ordo_campaign*`), publisher/consumer wiring jest potwierdzony poprawny (domyślne połączenie kolejki to `'db'`, zob. `DefaultValueProvider::getConnection()`), ale wiadomość `ordo.automation.campaign.dispatch` w `queue_message_status` konsekwentnie kończy na statusie 2 (in progress) lub 4 (retry required) — nigdy 3 (complete) — bez żadnego odpowiadającego wpisu w exception.log/system.log/debug.log. `queue:consumers:start ... -vvv` dodane do testu, żeby złapać faktyczny (najwyraźniej gdzieś połykany) wyjątek konsumenta.
