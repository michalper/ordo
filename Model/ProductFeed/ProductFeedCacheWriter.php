 ...
 *
 * $feedCode is the caller's (a FeedGeneratorInterface::getFeedCode()) — shared by every feed
 * format rather than one writer per format, since the write itself (upsert cache row + append
 * run log) never varies by format.
 */
class ProductFeedCacheWriter
{
    ...
    public function writeSuccess(string $feedCode, int $storeId, string $xml, int $productCount): void
    {
        ...
        $connection->query(
            ...
            [$feedCode, $storeId, $xml, $productCount]
        );

        $this->appendRunLog($feedCode, $storeId, ProductFeedRunLog::STATUS_SUCCESS, $productCount, null);
    }

    public function writeError(string $feedCode, int $storeId, string $message): void
    {
        $connection = $this->resourceConnection->getConnection();
        $table = $this->resourceConnection->getTableName('ordo_product_feed_cache');
        ...
        $connection->query(
            ...
            [$feedCode, $storeId, substr($message, 0, 255)]
        );

        $this->appendRunLog($feedCode, $storeId, ProductFeedRunLog::STATUS_ERROR, 0, substr($message, 0, 255));
    }
