...
**Status: every row below is ✅ except one 🔴 (see §10) and sixteen ⬜ (the frequency-cap
structural case, the Campaign export row, and the dispatch dead-letter row in §1d, the
`RetryFailedCampaignActions` row in §11, the Segment Overlap page and Segment export row in §2,
the WhatsApp Template Body Text preview panel in §15, the two Template Test Send rows in §16, the
two Cron Run Log rows in §17, and the four Product feed rows in §18 — all unit-tested but no
