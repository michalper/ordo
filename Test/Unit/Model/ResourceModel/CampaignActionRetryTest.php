...
class CampaignActionRetryTest extends AbstractDbTestCase
{
    public function testInitializesWithCampaignActionRetryTableAndEntityIdField(): void
    {
        $resource = new CampaignActionRetryResource($this->makeDbContext());

        self::assertSame('ordo_campaign_action_retry', $resource->getMainTable());
        self::assertSame('entity_id', $resource->getIdFieldName());
    }

    /**
     * Same atomic-claim-before-execute reasoning as ResourceModel\Campaign\ScheduledAction::claim()
    ...
    public function testClaimUpdatesModelAndReturnsTrueWhenRowWasStillDue(): void
    {
        ...
        $connection->expects(self::once())->method('update')
            ->with(
                'ordo_campaign_action_retry',
                ['next_retry_at' => '2026-01-01 00:05:00'],
                'entity_id = 5 AND next_retry_at <= 2026-01-01 00:00:00'
            )
            ->willReturn(1);

        $resource = $this->getMockBuilder(CampaignActionRetryResource::class)
            ->setConstructorArgs([$this->makeDbContext()])
            ->onlyMethods(['getConnection'])
            ->getMock();
        $resource->method('getConnection')->willReturn($connection);

        $model = $this->createMock(CampaignActionRetry::class);
