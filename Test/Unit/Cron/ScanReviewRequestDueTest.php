...
class ScanReviewRequestDueTest extends TestCase
{
    ...
    public function testExecuteSkipsWhenDisabled(): void
    {
        ...
        $config->method('isReviewRequestEnabled')->willReturn(false);
    ...
    public function testExecuteDispatchesTriggerForDueOrder(): void
    {
        $config = $this->createStub(Config::class);
        $config->method('isReviewRequestEnabled')->willReturn(true);
        $config->method('getReviewRequestDelayDays')->willReturn(14);

        $connection = $this->makeConnection([
            ['order_id' => 42, 'customer_id' => 5],
        ]);
        ...
        $dispatcher->expects(self::once())->method('dispatch')
            ->with(CampaignTriggerInterface::TRIGGER_REVIEW_REQUEST_DUE, [
                'customer_id' => 5,
                'order_id' => 42,
            ]);

        $logger = $this->createMock(LoggerInterface::class);
        $logger->expects(self::once())->method('info')->with(self::stringContains('1 review request triggers'));
    ...
    public function testExecuteDispatchesNothingWhenNoRowsFound(): void
    {
        ...
        $logger = $this->createMock(LoggerInterface::class);
        $logger->expects(self::once())->method('info')->with(self::stringContains('0 review request triggers'));
    ...
    ): ScanReviewRequestDue {
        return new ScanReviewRequestDue(
            $config,
            $resourceConnection,
