...
use Ordo\Automation\Ui\Component\Listing\Column\ContentBlockActions;
use PHPUnit\Framework\TestCase;

class ContentBlockActionsTest extends TestCase
{
    ...
    public function testPrepareDataSourceAddsEditAndDeleteLinks(): void
    {
        ...
        $urlBuilder->method('getUrl')->willReturnMap([
            ['ordo/contentblock/edit', ['entity_id' => 5], 'https://example.com/admin/ordo/contentblock/edit/entity_id/5/'],
            ['ordo/contentblock/delete', ['entity_id' => 5], 'https://example.com/admin/ordo/contentblock/delete/entity_id/5/'],
        ]);

        $context = $this->createStub(ContextInterface::class);
        $context->method('getProcessor')->willReturn($this->createStub(Processor::class));

        $column = new ContentBlockActions($context, $this->createStub(UiComponentFactory::class), $urlBuilder);
        $column->setData('name', 'actions');

        $dataSource = [
            'data' => [
                'items' => [
                    ['entity_id' => 5, 'name' => 'Welcome snippet'],
                ],
            ],
        ];
    ...
    private function makeColumn(): ContentBlockActions
    {
        $context = $this->createStub(ContextInterface::class);
        $context->method('getProcessor')->willReturn($this->createStub(Processor::class));

        return new ContentBlockActions(
            $context,
