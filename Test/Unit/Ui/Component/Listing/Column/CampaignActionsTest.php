...
use Ordo\Automation\Ui\Component\Listing\Column\CampaignActions;
use PHPUnit\Framework\TestCase;

class CampaignActionsTest extends TestCase
{
    ...
    public function testPrepareDataSourceAddsEditAndDeleteLinks(): void
    {
        ...
        $column = new CampaignActions($context, $this->createStub(UiComponentFactory::class), $urlBuilder);
        $column->setData('name', 'actions');

        $dataSource = [
            'data' => [
                'items' => [
                    ['entity_id' => 5, 'name' => 'Welcome'],
                ],
            ],
        ];
    ...
    private function makeColumn(): CampaignActions
    {
        $context = $this->createStub(ContextInterface::class);
        $context->method('getProcessor')->willReturn($this->createStub(\Magento\Framework\View\Element\UiComponent\Processor::class));

        return new CampaignActions(
            $context,
